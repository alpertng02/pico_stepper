...
class RoboticArm {
  ...
  using StepperRads = std::array<float, 3>;
...
public:
  RoboticArm(uint pul1 = 2, uint dir1 = 3, uint pul2 = 6, uint dir2 = 7,
    ...
    auto armRads = getArmRads(initPx, initPy, initT1);
    for (int i = 0; i < 3; i++) {
  ...
  void move(const float px, const float py, const float theta, const float sec,
    ...
    for (int i = 0; i < 3; i++) {
      float accel = std::clamp(fabsf(rads[i] - stepper[i].getPosRads() * gain),
                               0.f, maxAccels[i]);

      stepper[i].startMotion(rads[i], accel, sec);
    }
  }

  void setCurrentPosition(const float px, const float py, const float theta) {
    auto armRads = getArmRads(px, py, theta);
    for (int i = 0; i < 3; i++) {
      stepper[i].setPos(armRads[i]);
    }
  }

  StepperRads getCurrentPosition() {
    return {stepper[0].getPosRads(), stepper[1].getPosRads(),
            stepper[2].getPosRads()};
  }
...
private:
  ...
  StepperRads getArmRads(const float px, const float py, const float theta) {
...
int main() {
  ...
  while (true) {
    ...
    arm.move(px, py, theta, sec, 1.0f);
    sleep_ms(10);
    printf("Current Position => ");
    for (const auto &rad : arm.getCurrentPosition()) {
      printf("%.4f\t", rad);
    }
    puts("");
  }
